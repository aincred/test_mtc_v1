...
import { useRouter } from "next/navigation";
import { 
  Search, Download, FileText, ArrowLeft, Eye, Filter, User, Activity, Loader2, Calendar, SlidersHorizontal 
} from "lucide-react";
import toast, { Toaster } from "react-hot-toast";
import { format } from "date-fns";
...
export default function ChildRecordsPage() {
  ...
  const exportToCSV = () => {
    ...
    const csvData = filteredRecords.map(r => [
      r.samNumber, r.recordNo, r.childName, r.parentName, r.mobileNumber,
      r.sex === "1" ? "Male" : "Female", r.admissionDate, r.admissionWeight, 
      r.admissionHeight, r.zScore, r.village || "N/A", r.dischargeDate ? "Discharged" : "Active" 
    ]);
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] py-4 md:py-8 px-4 sm:px-6 lg:px-8">
      <Toaster position="top-center" />
      
      <div className="max-w-7xl mx-auto">
        {/* Header Section */}
        <div className="mb-4 flex items-center">
          <Button variant="ghost" onClick={() => router.back()} className="pl-0 text-slate-500 h-8">
            <ArrowLeft className="w-5 h-5 mr-2" /> Back
          </Button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-6 md:mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 flex items-center gap-2 md:gap-3">
              <FileText className="text-indigo-600 h-6 w-6 md:h-8 md:w-8 shrink-0" />
              Master Records
            </h1>
            <p className="mt-1 md:mt-2 text-xs md:text-sm text-slate-500 max-w-2xl font-medium">
              Database for: <span className="font-bold text-indigo-600 uppercase tracking-wider">{mtcName || "Loading..."}</span>
            </p>
          </div>
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button 
              ...
              variant="outline" 
              className="flex-1 sm:hidden border-slate-200 bg-white"
            >
            ...
            <Button 
              onClick={exportToCSV} 
              variant="outline" 
              className="flex-1 sm:flex-none border-indigo-200 text-indigo-700 hover:bg-indigo-50 bg-white"
            >
        ...
        {/* Toolbar: Search, Status, and Date Filters */}
        <div className={cn(
          "mb-6 transition-all duration-300 ease-in-out origin-top",
        ...
        <Card className="border-0 shadow-sm border-t border-slate-200 relative min-h-[400px] bg-slate-50 md:bg-white">
          {isLoading && (
          ...
          {!isLoading && filteredRecords.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-4 bg-white rounded-2xl">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-100 mb-4">
                <Search className="h-8 w-8 text-slate-400" />
              </div>
              <h3 className="text-lg font-medium text-slate-900 mb-1">No records found</h3>
            ...
            <>
              ...
              <div className="md:hidden p-4 space-y-4">
                {filteredRecords.map((record) => (
                  <div key={record.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3">
                    <div className="flex justify-between items-start mb-1">
                      <div>
                        <div className="font-mono text-xs font-semibold text-indigo-700 bg-indigo-50 inline-block px-2 py-1 rounded mb-2">
                          {record.samNumber}
                        </div>
                        ...
                        <p className="text-slate-500 text-xs mt-1">C/O: {record.parentName}</p>
                      </div>
                      <span className={cn(
                        "inline-flex items-center px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border shrink-0 mt-1",
                        record.dischargeDate 
                    ...
                    <div className="grid grid-cols-2 gap-2 p-3 bg-slate-50 rounded-lg text-xs">
                      <div>
                        <p className="text-slate-400 mb-1 font-medium">Admission</p>
                      ...
                      <div>
                        <p className="text-slate-400 mb-1 font-medium">Vitals (Adm)</p>
                        <div className="flex items-center gap-1.5 text-slate-700 font-semibold">
                          <Activity size={12} className="text-teal-500" />
                          {record.admissionWeight} kg <span className="text-slate-300 font-normal">|</span> <span className="text-red-500">Z: {record.zScore}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <Button 
                      variant="outline" 
                      className="w-full mt-1 border-indigo-100 text-indigo-700 hover:bg-indigo-50 bg-white"
                      onClick={() => handleViewDetails(record.id)}
                    >
              ...
              <div className="hidden md:block overflow-x-auto bg-white rounded-t-2xl">
                <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 font-semibold">
                    <tr>
                      <th className="px-4 lg:px-6 py-4">SAM Number</th>
                      <th className="px-4 lg:px-6 py-4">Patient Details</th>
                      <th className="px-4 lg:px-6 py-4 hidden lg:table-cell">Admission Details</th>
                      <th className="px-4 lg:px-6 py-4">Vitals (Adm.)</th>
                    ...
                    {filteredRecords.map((record) => (
                      <tr key={record.id} className="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td className="px-4 lg:px-6 py-4">
                          ...
                          <div className="text-slate-400 text-xs mt-0.5 hidden xl:block">
                            {record.village ? `Village: ${record.village}` : 'Address not provided'}
                          </div>
                        </td>
                        
                        {/* Admission Info (Hidden on Tablet, visible on Desktop) */}
                        ...
                        <td className="px-4 lg:px-6 py-4 text-right">
                          <Button 
                            ...
                            size="sm"
                            onClick={() => handleViewDetails(record.id)}
                            className="opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity text-indigo-600 hover:text-indigo-800 hover:bg-indigo-50"
                          >
              ...
              {/* Bottom Footer Info */}
              <div className="bg-slate-50 px-6 py-4 md:border-t border-slate-200 text-sm text-slate-500 flex justify-between items-center rounded-b-2xl">
                <span>Showing <span className="font-bold text-slate-900">{filteredRecords.length}</span> records for {mtcName || "All"}</span>
              </div>
            </>
          )}
        </Card>
