...
import { Input } from "@/components/ui/input";
import { Card, CardHeader, CardContent } from "@/components/ui/card";
import { CalendarIcon, Home, Save, X, Loader2, Activity, ListFilter } from "lucide-react";
import toast, { Toaster } from "react-hot-toast";
...
export default function BedOccupancyPage() {
  ...
  // Mobile UI specific state
  const [selectedMobileMonth, setSelectedMobileMonth] = useState<number>(new Date().getMonth());
  ...
  const handleEdit = (record: BedOccupancyRecord) => {
    ...
    // Switch mobile view to the correct month automatically
    setSelectedMobileMonth(record.month - 1);
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-4 md:py-8 lg:py-10 px-4 md:px-6 lg:px-8 font-sans">
      <Toaster position="top-center" toastOptions={{ className: 'rounded-xl shadow-lg font-medium text-sm md:text-base' }} />
      
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        
        {/* RESPONSIVE HEADER: Stacked on mobile, row on Tab/PC */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
          <div className="flex flex-col md:flex-row items-center gap-3">
            <div className="bg-indigo-100 p-2.5 rounded-xl border border-indigo-200 shadow-sm hidden md:block">
              <Activity className="h-6 w-6 text-indigo-700" />
            </div>
            <div>
              ...
              <p className="text-sm font-medium text-slate-500 mt-1">
                Center: <span className="font-bold text-indigo-600 uppercase tracking-wider">{mtcName || "Loading..."}</span>
              </p>
            </div>
          </div>
          <Button
            onClick={() => router.push("/mtc-user/dashboard/home")}
            variant="outline"
            className="w-full md:w-auto border-slate-200 text-slate-700 hover:bg-slate-100 transition shadow-sm bg-white"
          >
            <Home className="mr-2 h-4 w-4" /> Back to Dashboard
          </Button>
        </div>
        ...
        <Card className="shadow-sm border-0 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] overflow-visible">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-4 pt-5 px-4 md:px-6">
            <h2 className="text-sm md:text-base font-bold text-slate-800 flex items-center justify-center md:justify-start gap-2 uppercase tracking-wide">
              <CalendarIcon className="h-4 w-4 text-indigo-500" /> 
              Daily Entry 
              {editingId && <span className="text-xs text-rose-500 bg-red-50 px-2 py-0.5 rounded border border-rose-100 ml-2 normal-case tracking-normal">Editing</span>}
            </h2>
          </CardHeader>
          
          <CardContent className="p-4 md:p-6">
            {/* GRID: 1 col on Mobile, 2 cols on Tab, 4 cols on PC */}
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input
                    ...
                    value={date}
                    onChange={(e) => setDate(e.target.value)}
                    className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-500 pr-10 w-full"
                    disabled={editingId !== null} 
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input 
                    ...
                    value={isCapacityLoading ? "Fetching..." : (bedSanctioned ? `${bedSanctioned} Beds` : "N/A")}
                    readOnly
                    className="bg-slate-100 font-bold text-slate-600 cursor-not-allowed border-slate-200 w-full"
                  />
                  {isCapacityLoading && <Loader2 className="absolute right-3 top-2.5 h-4 w-4 animate-spin text-slate-400" />}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  placeholder={!bedSanctioned ? "Wait..." : "Enter count"}
                  min="0"
                  className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-500 font-semibold w-full"
                  onChange={(e) => {
                    const val = e.target.value;
                    if (val === "" || parseFloat(val) < 0) {
                      setUtilizedBed("");
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <Input 
                    value={bedOccupancyPercentage} 
                    readOnly 
                    className="bg-indigo-50 font-bold text-indigo-700 border-indigo-200 pr-8 w-full" 
                    placeholder="0.00"
                  />
                  <span className="absolute right-3 top-2.5 text-indigo-500 font-bold">%</span>
                </div>
            ...
            {/* BUTTONS: Stacked full-width on mobile, right-aligned row on tab/PC */}
            <div className="flex flex-col md:flex-row justify-end gap-3 mt-6 md:mt-8">
              <Button onClick={handleClear} variant="outline" className="border-slate-200 text-slate-600 hover:bg-slate-100 w-full md:w-auto order-2 md:order-1">
                <X className="mr-2 h-4 w-4" /> {editingId ? 'Cancel Edit' : 'Clear Form'}
              </Button>
              <Button onClick={handleSave} disabled={isSaving || isCapacityLoading} className="bg-indigo-600 hover:bg-indigo-700 w-full md:w-auto order-1 md:order-2 shadow-sm">
                {isSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}
                {editingId ? 'Update Record' : 'Save Record'}
              </Button>
            </div>
        ...
        <Card className="border-0 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] overflow-hidden">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100 pb-4 pt-5 px-4 md:px-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            <h2 className="text-sm md:text-base font-bold text-slate-800 flex items-center gap-2 uppercase tracking-wide">
              <Activity className="h-4 w-4 text-indigo-500" /> 
              Overview
            </h2>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <select
                value={selectedYear}
                onChange={(e) => setSelectedYear(e.target.value)}
                className="w-full sm:w-40 px-3 py-2 md:py-1.5 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-medium bg-white shadow-sm"
              >
                {yearOptions.map(year => (
                  <option key={year} value={year}>{year} Records</option>
                ))}
            ...
            <div className="block lg:hidden w-full">
              <div className="p-4 bg-slate-100/50 border-b border-slate-200 flex items-center gap-3">
                ...
                <select
                  ...
                  onChange={(e) => setSelectedMobileMonth(Number(e.target.value))}
                  className="flex-1 p-2.5 border border-slate-200 rounded-lg bg-white shadow-sm font-semibold text-slate-700"
                >
                  {monthlyData.map((m, i) => (
                    <option key={i} value={i}>{m.month} - {m.occupancy > 0 ? `${m.occupancy.toFixed(1)}% Avg` : 'No Data'}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="max-h-[500px] overflow-y-auto custom-scrollbar divide-y divide-slate-100">
                {dailyData.map((row, dayIndex) => {
                  const value = row[selectedMobileMonth];
                  return (
                    <div 
                      key={dayIndex} 
                      onClick={() => {
                      ...
                      className={`flex justify-between items-center p-4 transition-colors ${
                        value > 0 
                        ? 'bg-white active:bg-indigo-50 cursor-pointer hover:bg-slate-50' 
                        : 'bg-slate-50/30'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${value > 0 ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-500'}`}>
                          {dayIndex + 1}
                        </div>
                        <span className="text-sm font-semibold text-slate-600">Day {dayIndex + 1}</span>
                      </div>
                      <div className="text-right">
                        {value > 0 ? (
                          <span className="font-bold text-indigo-600 block">{Number(value).toFixed(1)}%</span>
                        ) : (
                          <span className="text-xs font-medium text-slate-400 uppercase tracking-wider block">No Entry</span>
                        )}
                        {value > 0 && <span className="text-[10px] text-slate-400 block mt-0.5">Tap to Edit</span>}
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="p-4 bg-indigo-50 border-t border-indigo-100 flex justify-between items-center shadow-[0_-4px_10px_-4px_rgba(0,0,0,0.05)] sticky bottom-0">
                <span className="font-bold text-indigo-900 uppercase tracking-wider text-xs">Monthly Average</span>
                <span className="font-bold text-indigo-700 text-lg">
                  {monthlyData[selectedMobileMonth]?.occupancy > 0 ? `${monthlyData[selectedMobileMonth].occupancy.toFixed(1)}%` : '-'}
                </span>
              </div>
            </div>

            {/* -----------------------------
                PC & TABLET VIEW (MATRIX)
                ...
                <table className="min-w-full text-sm text-slate-700 border-collapse">
                  <thead className="sticky top-0 z-20">
                    <tr className="bg-slate-50 shadow-sm">
                      <th className="py-3 px-4 text-left font-bold text-slate-500 uppercase tracking-wider text-xs border-r border-b border-slate-200 bg-slate-100 sticky left-0 z-30">
                        Day \ Month
                      </th>
                      {monthlyData.map((m, i) => (
                        <th key={i} className="py-3 px-4 text-center font-bold text-slate-500 uppercase tracking-wider text-xs border-b border-slate-200 bg-slate-50">
                          {m.month}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {dailyData.map((row, dayIndex) => (
                      <tr key={dayIndex} className="hover:bg-indigo-50/40 transition-colors group">
                        <th className="py-2.5 px-4 font-bold text-left bg-slate-50 border-r border-slate-200 text-slate-600 sticky left-0 z-10 group-hover:bg-indigo-50/40 transition-colors">
                          {dayIndex + 1}
                        </th>
                        {row.map((value, monthIndex) => (
                          <td 
                            key={monthIndex} 
                            className={`py-2.5 px-4 text-center border-l border-slate-50 ${
                              value > 0 
                              ? 'cursor-pointer font-bold text-indigo-700 hover:bg-indigo-100 hover:scale-[1.03] transition-all' 
                              : 'text-slate-300'
                            }`}
                            title={value > 0 ? "Click to edit" : ""}
                            onClick={() => {
                  ...
                  <tfoot className="sticky bottom-0 z-20">
                    <tr className="bg-indigo-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] border-t-2 border-indigo-200">
                      <th className="py-4 px-4 font-bold text-left border-r border-indigo-200 text-indigo-900 uppercase tracking-wider text-xs sticky left-0 z-30 bg-indigo-100">
                        Avg %
                      </th>
                      {monthlyData.map((data, index) => (
                        <td key={index} className="py-4 px-4 text-center font-bold text-indigo-700 border-l border-indigo-100/50 bg-indigo-50">
                          {data.occupancy > 0 ? `${data.occupancy.toFixed(1)}%` : "-"}
                        </td>
                      ))}
                    </tr>
