...
import { 
  HeartPulse, Apple, User, CheckCircle, ArrowLeft, 
  Activity, MessageCircle, Pill, ClipboardList, Loader2, Search
} from "lucide-react";
import toast, { Toaster } from "react-hot-toast";
import { clsx, type ClassValue } from "clsx";
import { twMerge } from "tailwind-merge";
...
export default function MaternalNutritionPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch currently admitted children
        const childRes = await fetch(`/api/child-registration${queryParams}`);
        if (childRes.ok) {
          const children = await childRes.json() as ChildDropdownItem[];
          setRegisteredChildren(children);
  ...
  const filteredChildren = useMemo(() => {
    if (!searchQuery.trim()) return registeredChildren;
    const q = searchQuery.toLowerCase();
    return registeredChildren.filter(child => 
      child.child_full_name.toLowerCase().includes(q) ||
      child.sam_no.toLowerCase().includes(q) ||
      child.guardian_name.toLowerCase().includes(q)
    );
  }, [registeredChildren, searchQuery]);
    ...
    return (
      <div className="min-h-screen bg-[#F8FAFC] flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 text-center border-0 shadow-lg">
          ...
          <h2 className="text-2xl font-extrabold text-slate-900 mb-2">Assessment Recorded!</h2>
          <p className="text-slate-500 mb-6">
            Maternal nutrition and counseling data for <span className="font-bold text-slate-800">{selectedChild?.guardian_name}</span> has been saved.
          </p>
          <div className="flex flex-col gap-3">
            <Button onClick={handleReset} className="w-full">Assess Another Mother</Button>
            <Button variant="outline" onClick={() => router.push("/mtc-user/dashboard/home")} className="w-full">Back to Home</Button>
        ...
        {isFetching ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Section 1: Search & Patient Selection */}
              <Card className="border-0 shadow-sm border-t-4 border-t-indigo-500">
                <CardContent className="p-6">
                  <SectionTitle icon={User} title="Link to Admitted Child" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2 space-y-3">
                      <Label>Search & Select Admitted Child <span className="text-red-500">*</span></Label>
                      ...
                      <div className="relative">
                        ...
                        <Input
                          type="text"
                          placeholder="Type child name, SAM number, or guardian..."
                          value={searchQuery}
                      ...
                      <select
                        ...
                        required
                        className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 focus:bg-white transition-all duration-200"
                      >
                        <option value="" disabled>-- Select Patient from Results --</option>
                        {filteredChildren.length === 0 ? (
                          <option value="" disabled>No matching records found</option>
                        ) : (
                          filteredChildren.map((child) => (
                            <option key={child.registration_id} value={child.registration_id.toString()}>
                              {child.child_full_name} (SAM: {child.sam_no}) — Caregiver: {child.guardian_name}
                            </option>
                          ))
                        )}
                    ...
                    <div>
                      <Label>Lactating Mother? <span className="text-red-500">*</span></Label>
                      <select name="lactatingStatus" required className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
              ...
              <Card className="border-0 shadow-sm">
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      <Label>Meals per Day <span className="text-red-500">*</span></Label>
                      <select name="mealsPerDay" required className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
                        <option value="">Select frequency</option>
                        <option value="1-2 meals">1-2 meals</option>
                        <option value="3 meals">3 meals</option>
                        <option value="4+ meals">4 or more meals</option>
                      </select>
                    </div>
                    
                    <div>
                      <Label className="flex items-center gap-2"><Pill size={14}/> IFA Tablets Given? <span className="text-red-500">*</span></Label>
                      <select name="ifaGiven" required className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
                        <option value="">Select</option>
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                        <option value="already_taking">Already Taking</option>
                      </select>
                    </div>

                    <div>
                      <Label className="flex items-center gap-2"><Pill size={14}/> Calcium Tablets Given? <span className="text-red-500">*</span></Label>
                      <select name="calciumGiven" required className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30">
                        <option value="">Select</option>
                        <option value="yes">Yes</option>
                        <option value="no">No</option>
                        <option value="already_taking">Already Taking</option>
              ...
              <Card className="border-0 shadow-sm">
                <CardContent className="p-6">
                  ...
                  <Label className="mb-4 block">Select all topics covered during this session:</Label>
                  <div className="bg-slate-50 p-5 rounded-xl border border-slate-100">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-4 gap-x-6">
                      {[
                        { id: "iycf", label: "Infant & Young Child Feeding (IYCF)" },
                      ...
                      {savedRecords.length > 0 ? (
                        savedRecords.map((record) => (
                          <tr key={record.id} className="hover:bg-slate-50/80 transition-colors">
                            ...
                            <td className="px-6 py-4 text-center capitalize text-slate-700">
                              {record.ifaGiven?.replace("_", " ")}
                            </td>
                            <td className="px-6 py-4 text-center capitalize text-slate-700">
                              {record.calciumGiven?.replace("_", " ")}
                            </td>
                          </tr>
                        ))
                      ) : (
