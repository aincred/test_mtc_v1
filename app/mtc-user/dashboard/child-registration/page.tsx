...
export default function ChildRegistrationPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-4 sm:py-6 md:py-8 lg:py-10 px-4 sm:px-6 lg:px-8">
      <Toaster position="top-right" />
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex w-full md:w-auto gap-3">
            <Button
              onClick={handleBackToHome}
              variant="outline"
              className="flex-1 md:flex-none border-gray-300 text-gray-700 hover:bg-gray-100 transition"
            >
            ...
            <Button
              onClick={handleAdd}
              className="flex-1 md:flex-none bg-indigo-600 hover:bg-indigo-700 transition"
            >
          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 items-end">
              <div>
                ...
                <div className="relative">
                  <Input type="date" value={fromDate} onChange={(e) => setFromDate(e.target.value)} className="pr-10 w-full" />
                  <CalendarIcon className="absolute right-3 top-2.5 text-gray-400 h-4 w-4" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">To Date</label>
                <div className="relative">
                  <Input type="date" value={toDate} onChange={(e) => setToDate(e.target.value)} className="pr-10 w-full" />
                  <CalendarIcon className="absolute right-3 top-2.5 text-gray-400 h-4 w-4" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Record No</label>
                <Input placeholder="Enter Record No" value={recordNo} onChange={(e) => setRecordNo(e.target.value)} className="w-full" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">SAM Number</label>
                <Input placeholder="Enter SAM Number" value={samNumber} onChange={(e) => setSamNumber(e.target.value)} className="w-full" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Child Name</label>
                <div className="flex gap-2">
                  <Input placeholder="Enter Child Name" value={childName} onChange={(e) => setChildName(e.target.value)} className="w-full" />
                  <Button onClick={handleSearch} className="bg-indigo-600 hover:bg-indigo-700 px-3">
        ...
        {/* Swipeable Tabs for Mobile / Flex on Desktop */}
        <div className="flex overflow-x-auto gap-2 items-center bg-white p-1.5 rounded-xl border border-gray-200 shadow-sm w-full lg:w-max [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          <button
            onClick={() => setViewType("all")}
            className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-lg transition-all ${viewType === "all" ? "bg-slate-800 text-white shadow" : "text-gray-600 hover:bg-gray-100"}`}
          >
            All Patients <span className="ml-1.5 text-xs opacity-70">({countAll})</span>
          </button>
          <button
            onClick={() => setViewType("normal")}
            className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-lg transition-all ${viewType === "normal" ? "bg-blue-600 text-white shadow" : "text-gray-600 hover:bg-gray-100"}`}
          >
            Normal Registration <span className="ml-1.5 text-xs opacity-70">({countNormal})</span>
          </button>
          <button
            onClick={() => setViewType("samar")}
            className={`whitespace-nowrap px-4 py-2 text-sm font-semibold rounded-lg transition-all flex items-center gap-2 ${viewType === "samar" ? "bg-purple-600 text-white shadow" : "text-gray-600 hover:bg-gray-100"}`}
          >
            <CheckCircle2 className="w-4 h-4" />
            SAAMAR Tracker <span className="ml-0.5 text-xs opacity-70">({countSamar})</span>
          </button>
        ...
        <Card className="shadow-sm border border-gray-200 bg-transparent sm:bg-white border-none sm:border-solid shadow-none sm:shadow-sm">
          <CardHeader className="hidden sm:flex pb-4 flex-row items-center justify-between">
          ...
          <CardContent className="p-0 sm:p-6">
            {loading ? (
              <div className="py-12 bg-white rounded-lg border border-gray-200 text-center">
                <div className="flex flex-col items-center justify-center">
                  <Loader2 className="h-8 w-8 animate-spin text-indigo-500 mb-4" />
                  <p className="font-medium text-gray-600">Loading database records...</p>
                </div>
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:hidden gap-4">
                  {filteredData.map((child, i) => (
                    <Card key={child.id || i} className="hover:border-indigo-300 transition-colors bg-white shadow-sm">
                      <CardContent className="p-4">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h3 className="font-semibold text-gray-900 text-base">{child.childName}</h3>
                            <p className="text-xs text-gray-500 font-mono mt-0.5">Record: {child.recordNo} | SAM: {child.samNumber}</p>
                          </div>
                          <Button 
                            variant="outline" 
                            size="sm" 
                            onClick={() => handleEdit(child.id)} 
                            className="h-8 w-8 p-0 shrink-0 border-red-200 hover:bg-red-50 hover:border-red-300"
                          >
                        ...
                        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm mb-4">
                          <div>
                            <span className="text-gray-500 text-xs block mb-0.5">Parent Name</span>
                            <span className="text-gray-900 font-medium truncate block" title={child.parentName}>{child.parentName}</span>
                          </div>
                          <div>
                            <span className="text-gray-500 text-xs block mb-0.5">Date of Birth</span>
                            <span className="text-gray-900 font-medium">{child.dateOfBirth}</span>
                          </div>
                          <div>
                            <span className="text-gray-500 text-xs block mb-0.5">Weight / Height</span>
                            <span className="text-gray-900 font-medium">{child.admissionWeight} kg / {child.admissionHeight} cm</span>
                          </div>
                        </div>

                        <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                          <span className="text-xs font-medium text-gray-500">Registration Type</span>
                          {child.isSamarRegistered ? (
                            <div className="text-right flex flex-col items-end">
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold tracking-widest uppercase bg-purple-100 text-purple-700">
                                SAAMAR
                              </span>
                              <span className="text-[10px] text-gray-400 mt-1 font-mono">{child.samarUuid || "No UUID"}</span>
                            </div>
                          ) : (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold tracking-widest uppercase bg-gray-100 text-gray-600">
                              Normal
                            </span>
                          )}
                        </div>
                ...
                {/* === DESKTOP PC VIEW (Table) === */}
                <div className="hidden lg:block overflow-x-auto rounded-lg border border-gray-200">
                  <table className="min-w-full text-sm text-gray-700 border-collapse">
                    <thead>
                      <tr className="bg-indigo-50/50 text-indigo-800 border-b border-gray-200">
                        <th className="py-3.5 px-4 text-left font-semibold">Record No</th>
                        <th className="py-3.5 px-4 text-left font-semibold">SAM Number</th>
                        <th className="py-3.5 px-4 text-left font-semibold">Child Name</th>
                        <th className="py-3.5 px-4 text-left font-semibold">Registration Type</th>
                        <th className="py-3.5 px-4 text-left font-semibold">Parent Name</th>
                        <th className="py-3.5 px-4 text-left font-semibold">Date of Birth</th>
                        <th className="py-3.5 px-4 text-left font-semibold">Weight</th>
                        <th className="py-3.5 px-4 text-left font-semibold">Height</th>
                    ...
                    <tbody>
                      {filteredData.map((child, i) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-4 text-gray-700">{child.parentName}</td>
                          <td className="py-3 px-4 text-gray-700">{child.dateOfBirth}</td>
                          <td className="py-3 px-4 text-gray-700">{child.admissionWeight} kg</td>
                          <td className="py-3 px-4 text-gray-700">{child.admissionHeight} cm</td>
                          <td className="py-3 px-4 text-center">
                            <div className="flex justify-center">
                              <Button
                                variant="outline"
                                size="sm"
                                className="border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 hover:border-red-300 p-2 transition-colors"
                                onClick={() => handleEdit(child.id)}
                              >
                                <Pencil className="h-4 w-4" />
                              </Button>
              ...
              <div className="py-12 bg-white rounded-lg border border-gray-200 text-center text-gray-500">
                <div className="flex flex-col items-center">
                  <div className="mb-3 bg-gray-100 p-3 rounded-full">
                    <Search className="w-6 h-6 text-gray-400" />
                  </div>
                  <p className="font-medium text-gray-900 text-lg">No children found</p>
                  <p className="mt-1 text-sm">Adjust your filters or add a new registration.</p>
                </div>
              </div>
            )}
