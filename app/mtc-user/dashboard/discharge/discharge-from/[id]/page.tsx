...
export default function DischargeFormPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ✅ DYNAMIC Z-SCORE STRING CATEGORIZATION (Fixed to override raw numbers like 4.5)
  useEffect(() => {
    if (dischargeWeight && dischargeHeight && child?.Sex) {
      const weight = parseFloat(dischargeWeight);
      const height = parseFloat(dischargeHeight);
      
      try {
        const rawScore = calculateZScore(weight, height, child.Sex);
        let formattedScore = "Error";
        
        // Ensure we handle both numbers and string-represented numbers (like "4.5")
        const numericScore = typeof rawScore === 'string' ? parseFloat(rawScore) : rawScore;
        
        if (typeof numericScore === "number" && !isNaN(numericScore)) {
          if (numericScore >= -1) formattedScore = "> -1SD";
          else if (numericScore >= -2) formattedScore = "> -2SD";
          else if (numericScore >= -3) formattedScore = "> -3SD";
          else formattedScore = "> -4SD";
        } 
        // Fallback for strict strings returned from the utility (e.g. "> -2SD" explicitly)
        else if (typeof rawScore === "string") {
          formattedScore = rawScore;
        }

        setDischargeZScore(formattedScore);
      } catch (error) {
        console.error("Z-Score calculation error:", error);
        setDischargeZScore("Error");
      }
    } else {
      setDischargeZScore("");
    }
  }, [dischargeWeight, dischargeHeight, child?.Sex]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        DischargeHeight: parseFloat(dischargeHeight),
        DischargeMuac: parseFloat(dischargeMuac),
        // ✅ Retains and sends the exact string (e.g. "> -2SD") to DB
        DischargeZScore: dischargeZScore !== "Error" ? dischargeZScore : null,
        DischargeEdema: parseInt(dischargeEdema),
        ExitIndicator: parseInt(outcomeIndicator),
        IFAToMotherTablet: parseInt(ifaGivenToMother),
        MotherWages: parseInt(motherPayment),
        ...
        <Card className="shadow-xl border-0 rounded-2xl overflow-hidden bg-white">
          ...
          <CardContent className="p-8">
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              {/* --- SECTION 1: Read Only Details --- */}
              <div className="bg-slate-50/50 rounded-xl p-6 border border-slate-100 shadow-lg shadow-red-500/40">
                <div className="flex items-center justify-between mb-6">
                  <h6 className="flex items-center gap-2 text-sm font-bold text-green-600 uppercase tracking-wider">
                    <User className="h-4 w-4" /> Admission Information
                  </h6>
                  {child.isSamarRegistered && (
                    <span className="bg-purple-100 text-purple-700 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest border border-purple-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div>
                      <label className="text-xs font-semibold text-slate-500 uppercase">Sex</label>
                      <Input value={child.Sex ?? "N/A"} readOnly className="bg-slate-100 border-transparent text-slate-700 font-medium mt-1 focus-visible:ring-0" />
                  </div>
                  <div>
                      <label className="text-xs font-semibold text-slate-500 uppercase">Admission Date</label>
                      <div className="relative mt-1">
