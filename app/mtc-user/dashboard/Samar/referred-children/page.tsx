...
// Format CamelCase or database keys to readable labels
const formatColumnName = (col: string) => {
  if (col === "uuidChild") return "UUID Child";
  if (col === "AwcName") return "AWC Name";
  if (col === "AwcSevikaName") return "AWC Sevika Name";
...
// Retrieve logged-in user credentials from session storage
const getUserLocationContext = () => {
  if (typeof window === "undefined") return { district: "", mtcName: "" };
  try {
    const storedUser = sessionStorage.getItem("user");
    if (!storedUser) return { district: "", mtcName: "" };
    const user = JSON.parse(storedUser);
    return {
      district: (user.district || "").toString().trim().toLowerCase(),
      mtcName: (user.mtcName || "").toString().trim().toLowerCase(),
    };
  } catch {
    return { district: "", mtcName: "" };
  }
};
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (userDistrict || userMtcName) {
        rows = rows.filter((r: DataRow) => {
          const rowDistrict = (r.DistrictName || r.district || "").toString().trim().toLowerCase();
          const rowMtc = (r.MtcName || r.mtcName || r.MTCName || "").toString().trim().toLowerCase();
          const rowProject = (r.ProjectName || "").toString().trim().toLowerCase();

          // 1. If login is Ranchi, restrict strictly to Bundu records
          if (userDistrict.includes("ranchi") || userMtcName.includes("ranchi")) {
            return rowMtc.includes("bundu") || rowProject.includes("bundu");
          }

          // 2. If login is Chas/Bokaro, restrict strictly to Chas records
          if (userDistrict.includes("chas") || userMtcName.includes("chas") || userDistrict.includes("bokaro")) {
            return rowMtc.includes("chas") || rowProject.includes("chas");
          }

          // 3. Fallback matching against user's assigned district or MTC
          const matchesDistrict = userDistrict && rowDistrict.includes(userDistrict);
          const matchesMtc = userMtcName && (rowMtc.includes(userMtcName) || rowProject.includes(userMtcName));

          return matchesDistrict || matchesMtc;
        });
      }
