...
export async function GET(
  ...
  try {
    ...
    // Fetch the child's admission data. 
    // We join with mtc_admission_odema to get the text value (e.g., "No", "++")
    // ✅ Added SAAMAR tracking fields, sex, demographic data, and fixed parent names
    const sqlText = `
      SELECT 
        c.registration_id AS "id",
        c.sam_no AS "SamNo",
        ...
        c.sex_id AS "SexId",
        CASE 
          WHEN c.sex_id = 1 THEN 'Male'
          WHEN c.sex_id = 2 THEN 'Female'
          ELSE 'Other/Unknown' 
        END AS "Sex",
        TO_CHAR(c.dob, 'YYYY-MM-DD') AS "DateOfBirth",
        c.age_years AS "AgeYears",
        ...
        TO_CHAR(c.admission_date, 'YYYY-MM-DD') AS "AdmissionDate",
        c.admission_weight_kg AS "AdmissionWeight",
        c.length_height_cm AS "AdmissionHeight",
        c.muac_cm AS "AdmissionMuac",
        c.z_score_sd AS "ZScore",
        c.admission_hemoglobin AS "AdmissionHemoglobin",
