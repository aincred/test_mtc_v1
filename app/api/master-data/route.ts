...
const formatLookupData = (rows: Record<string, unknown>[]) => {
  return rows.map(row => {
    ...
    // Fallback logic to robustly find the ID and Name keys independent of object property order
    const idKey = keys.find(k => k.toLowerCase().includes('id')) || keys[0];
    const nameKey = keys.find(k => k.toLowerCase().includes('name') || k.toLowerCase().includes('desc')) || keys[1];
...
export async function GET() {
  try {
    // Explicitly defining aliases (AS id, AS name) to ensure predictable object key formats
    const admissionTypes = await query('SELECT admission_type_id AS id, admission_type_name AS name FROM mtc_admission_type ORDER BY 1 ASC');
    const referredBy = await query('SELECT referred_by_id AS id, referred_by_name AS name FROM mtc_referred_by ORDER BY 1 ASC');
    const castes = await query('SELECT caste_id AS id, caste_name AS name FROM mtc_caste ORDER BY 1 ASC');
    const districts = await query('SELECT district_id AS id, district_name AS name FROM mtc_district ORDER BY 2 ASC');
    const sexes = await query('SELECT sex_id AS id, sex_name AS name FROM mtc_sex ORDER BY 1 ASC');
    const relationships = await query('SELECT relationship_id AS id, relationship_name AS name FROM mtc_relationship_with_child ORDER BY 1 ASC');
    const odemas = await query('SELECT odema_id AS id, odema_name AS name FROM mtc_admission_odema ORDER BY 1 ASC');
    const breastFeeding = await query('SELECT breast_feeding_id AS id, breast_feeding_name AS name FROM mtc_breast_feeding ORDER BY 1 ASC');
    const appetiteTests = await query('SELECT appetite_test_id AS id, appetite_test_name AS name FROM mtc_appetite_test ORDER BY 1 ASC');
    
    // Remaining ones that were already correctly mapped
    const blocks = await query('SELECT block_id AS id, block_name AS name, district_id AS "districtId" FROM mtc_block ORDER BY block_name ASC');
    const icdsProjects = await query('SELECT icds_id AS id, icds_name AS name FROM mtc_icds_project ORDER BY icds_name ASC');
    const anganwadis = await query('SELECT awd_id AS id, awd_name AS name FROM mtc_anganwadi ORDER BY awd_name ASC');
