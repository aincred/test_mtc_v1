...
export async function GET(request: Request) {
  try {
    ...
    // 1. Fetch MTC facilities for this district, join with bed capacity, and select doctor info
    const mtcSql = `
      SELECT 
        c.mtc_id AS id,
        c.mtc_name AS name,
    ...
    const locations = mtcResult.rows.map((row) => ({
      ...
      name: row.name,
      beds: Number(row.beds),
      mo: row.doctor_name || "Not Assigned", // Use database value or fallback
      contact: row.contact_number || "N/A"   // Use database value or fallback
    }));
