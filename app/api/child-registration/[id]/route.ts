...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const registrationId = parseNum(id);
    ...
    const sqlText = `SELECT * FROM mtc_child_master WHERE registration_id = $1`;
    const result = await query<MtcChildRow>(sqlText, [registrationId]);
    ...
    const patientData = {
      ...
      icdsProject: row.icds_project_id?.toString(),
      anganwadiCenter: row.anganwadi_id?.toString(),
      dateOfBirth: formatDate(row.dob),
      ageYears: row.age_years,
      ageMonths: row.age_months,
      sex: row.sex_id?.toString(),
  ...
  try {
    ...
    const sqlText = `
      UPDATE mtc_child_master SET
        mtc_id = $1, is_samar_registered = $2, samar_uuid = $3, admission_type_id = $4, referred_by_id = $5, 
        referred_by_name = $6, referred_by_mobile = $7, admission_date = $8, admission_time = $9, 
        child_full_name = $10, mother_name = $11, father_name = $12, dob = $13, age_years = $14, age_months = $15,
        sex_id = $16, guardian_name = $17, relationship_id = $18, mobile_number = $19, parent_aadhaar_number = $20, 
        bpl_number = $21, caste_id = $22, bank_name = $23, account_holder = $24, account_number = $25, ifsc_code = $26, 
        full_address = $27, district_id = $28, block_id = $29, village = $30, icds_project_id = $31, anganwadi_id = $32, 
        admission_weight_kg = $33, length_height_cm = $34, muac_cm = $35, z_score_sd = $36, odema_id = $37, 
        breast_feeding_id = $38, complementary_feeding = $39, appetite_test_id = $40, medical_complications = $41,
        updated_at = CURRENT_TIMESTAMP
    ...
    const values = [
      ...
      isCompFeeding, 
      parseNum(data.appetiteTest), 
      Array.isArray(data.medicalComplications) ? data.medicalComplications : [], 
      registrationId // $42
    ];
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const sqlText = `DELETE FROM mtc_child_master WHERE registration_id = $1 RETURNING registration_id`;
    const result = await query(sqlText, [registrationId]);
