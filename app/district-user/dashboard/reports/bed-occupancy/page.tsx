...
import { Search, Bed, Users, Percent, Download, Loader2, Building2 } from "lucide-react";
...
export default function BedOccupancyReport() {
  ...
  // Sync Logged-In User Session District
  useEffect(() => {
    const sessionData = sessionStorage.getItem("district_user") || sessionStorage.getItem("user");
    if (sessionData) {
      try {
        const user = JSON.parse(sessionData);
        if (user.districtName) setDistrictName(user.districtName.toUpperCase());
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/reports/bed-occupancy?${queryParams.toString()}`);
      const json = await res.json();

      if (!res.ok || !json.success) {
        throw new Error(json.error || json.message || "Failed to fetch occupancy report.");
      }
  ...
  const handleExportCSV = () => {
    ...
    csvContent += `MTC Bed Occupancy Report - ${districtName} District\n`;
    csvContent += `Range: ${range.toUpperCase()}, Year: ${year}\n\n`;
    csvContent += "MTC Name,Sanctioned Beds,Available Bed Days,Occupied Bed Days,Occupancy Rate (%)\n";

    reportData.forEach((row) => {
      csvContent += `"${row.mtcName}",${row.sanctionedBeds},${row.availableBedDays},${row.occupiedBedDays},${row.occupancyRate}\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `Bed_Occupancy_Report_${districtName}_${range}.csv`);
      ...
      <div className="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="p-6">
            ...
            {(["daily", "monthly", "quarterly"] as RangeType[]).map((r) => (
              <label key={r} className="flex items-center space-x-2 cursor-pointer capitalize">
                <input
                  ...
                  className="w-4 h-4 text-blue-600 focus:ring-blue-500 accent-blue-600"
                />
                <span className={`text-sm font-medium ${range === r ? 'text-blue-700 font-bold' : 'text-gray-600'}`}>{r}</span>
              </label>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end border-b border-gray-100 pb-8">
            ...
            <div>
              ...
              >
                ...
                {isSearching ? "Processing..." : "Generate Report"}
              </button>
            </div>
          </div>
            ...
            <div className="mt-8 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-center gap-4">
                  <div className="bg-blue-600 p-3 rounded-full text-white"><Bed size={24} /></div>
                  <div>
                    <p className="text-sm text-blue-800 font-medium">Total Sanctioned Beds</p>
                    <p className="text-2xl font-bold text-blue-900">{summary.totalSanctionedBeds}</p>
                  </div>
                </div>
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-center gap-4">
                  <div className="bg-blue-600 p-3 rounded-full text-white"><Users size={24} /></div>
                  <div>
                    <p className="text-sm text-blue-800 font-medium">Total Patient Days</p>
                    <p className="text-2xl font-bold text-blue-900">{summary.totalPatientDays}</p>
                  </div>
                </div>
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 flex items-center gap-4">
                  <div className="bg-blue-600 p-3 rounded-full text-white"><Percent size={24} /></div>
                  <div>
                    <p className="text-sm text-blue-800 font-medium">Avg. Occupancy Rate</p>
                    <p className="text-2xl font-bold text-blue-900">{summary.avgOccupancyRate}</p>
                  </div>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {reportData.length > 0 ? (
                      reportData.map((row, idx) => (
                        <tr key={idx} className="hover:bg-blue-50/30 transition-colors">
                          <td className="px-4 py-3 font-medium text-gray-900">{row.mtcName}</td>
                          <td className="px-4 py-3 text-center text-gray-600">{row.sanctionedBeds}</td>
                          <td className="px-4 py-3 text-center text-gray-600">{row.availableBedDays}</td>
                          <td className="px-4 py-3 text-center text-gray-600">{row.occupiedBedDays}</td>
                          <td className="px-4 py-3 text-right font-bold text-blue-700">{row.occupancyRate}</td>
                        </tr>
                      ))
                    ) : (
                      <tr>
                        <td colSpan={5} className="text-center py-8 text-gray-400">
                          No occupancy records found for the selected timeframe.
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div className="mt-6 flex justify-end">
                <button 
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-blue-700 text-white rounded-md text-sm font-bold hover:bg-blue-800 transition-colors shadow-sm"
                >
          ...
          {!showReport && !isSearching && (
            <div className="mt-12 text-center text-gray-400">
              <Bed size={48} className="mx-auto mb-4 opacity-20" />
              <p>Configure filters and click <span className="font-semibold text-blue-600">Generate Report</span> to view bed occupancy metrics.</p>
            </div>
          )}
        </div>
      </div>
