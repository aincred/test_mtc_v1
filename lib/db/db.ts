...
export interface DailyWeightData {
  ...
  SamNo: string;
  MTCCode: string;
  // Allow dynamic access for Day0...DayX, but restrict types to avoid 'any'
...
const parseSafeInt = (val: string | number | undefined | null): number | null => {
  if (!val) return null;
...
export async function saveFollowUpData(data: FollowUpData) {
  try {
    ...
    // Type alias for dynamic access safely
    type FollowUpSource = Record<string, string | number | Date | null | undefined>;

    const bindFollowUpInputs = (prefix: string, source: FollowUpSource) => {
      const dbPrefix = prefix === "Fourth" ? "Fourt" : prefix; 
      
      const getDate = (val: string | number | Date | null | undefined) => (val && typeof val !== 'number') ? new Date(val) : null;
      
      request.input(`${prefix}FollowUpDate`, sql.Date, getDate(source[`${prefix}FollowUpDate`]));
      request.input(`${dbPrefix}FollowUpDoneOn`, sql.Date, getDate(source[`${dbPrefix}FollowUpDoneOn`]));
    ...
    // Cast data safely
    const recordData = data as unknown as FollowUpSource;

    bindFollowUpInputs('First', recordData);
    bindFollowUpInputs('Second', recordData);
    bindFollowUpInputs('Third', recordData);
...
export async function saveEquipmentData(data: EquipmentData) {
  try {
    ...
    equipmentCols.forEach(col => {
      ...
      // Force double cast via unknown to allow record mapping for known keys
      const val = ((data as unknown) as Record<string, number | undefined>)[col];
      const value = val !== undefined ? val : 0;
